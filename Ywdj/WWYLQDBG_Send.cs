...
{
    [Description("委外用料清单变更单审核--推送MES")]
    public class WWYLQDBG_Send : AbstractOperationServicePlugIn
    {
        public override void EndOperationTransaction(EndOperationTransactionArgs e)
        {
            ...
            foreach (DynamicObject entity in e.DataEntitys)
            {
                ...
                if (entity != null)
                {
                    ...
                    string sql = string.Format(@"/*dialect*/ exec yzx_wwylqdbgd '{0}'", FID);
                    DynamicObjectCollection data = DBUtils.ExecuteDynamicObject(this.Context, sql);
                    
                    string sqlzx = string.Format(@"/*dialect*/ exec yzx_wwylqdbgdEntry '{0}'", FID);
                    ...
                    Dbhple.Log.writestr("委外用料清单变更单：" + data[0]["outsourceOrderCode"].ToString(), body.ToString(), responseMsg.ToString(), "_委外用料清单变更单");
