...
{
    [Description("生产订单反执行至计划确认--推送MES")]
    public class SCDDFJA_Send : AbstractOperationServicePlugIn
    {
        public override void EndOperationTransaction(EndOperationTransactionArgs e)
        {
            ...
            foreach (DynamicObject entity in e.DataEntitys)
            {
                ...
                if (entity != null)
                {
                    ...
                    string sql = string.Format(@"/*dialect*/ exec yzx_scdd_fja '{0}'", FID);
                    ...
                    var client = new RestClient(config[0]["http"].ToString() + "/api/v1/mes/openApi/shop/task/re/open");
                    client.Timeout = -1;
                    ...
                    Dbhple.Log.writestr("生产订单反结案：" + data[0]["returnOuterId"].ToString(), body.ToString(), responseMsg.ToString(), "_生产订单反结案");
