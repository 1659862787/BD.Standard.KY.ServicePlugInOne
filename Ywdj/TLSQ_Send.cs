...
{
    [Description("退料申请单审核--推送MES")]
    public class TLSQ_Send : AbstractOperationServicePlugIn
    {
        public override void EndOperationTransaction(EndOperationTransactionArgs e)
        {
            ...
            foreach (DynamicObject entity in e.DataEntitys)
            {
                ...
                if (entity != null)
                {
                    ...
                    string sql = string.Format(@"/*dialect*/ exec yzx_cgtl '{0}'", FID);
                    DynamicObjectCollection data = DBUtils.ExecuteDynamicObject(this.Context, sql);
              
                    string sqlzx = string.Format(@"/*dialect*/ exec yzx_cgtlEntry '{0}'", FID);
                    ...
                    Dbhple.Log.writestr("采购退料申请：" + data[0]["refundCode"].ToString(), body.ToString(), responseMsg.ToString(), "_采购退料申请");
