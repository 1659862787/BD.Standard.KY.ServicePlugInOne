...
namespace BD.Standard.KY.ServicePlugInOneV23
{
    [Description("委外订单变更单审核--推送MES")]
    public class WWSCDDBG_Send : AbstractOperationServicePlugIn
    {
        public override void EndOperationTransaction(EndOperationTransactionArgs e)
        {
            ...
            foreach (DynamicObject entity in e.DataEntitys)
            {
                ...
                if (entity != null)
                {
                    ...
                    string sql = string.Format(@"/*dialect*/ exec yzx_wwscddbgd '{0}'", FID);
                    DynamicObjectCollection data = DBUtils.ExecuteDynamicObject(this.Context, sql);
                   
                    string sqlzx = string.Format(@"/*dialect*/ exec yzx_wwscddbgdEntry '{0}'", FID);
                    ...
                    var client = new RestClient(config[0]["http"].ToString() + "/api/v1/mps/openApi/outsourceOrder/syn");
                    client.Timeout = -1;
                    ...
                    Dbhple.Log.writestr("委外生产订单变更单：" + data[0]["outsourceOrderCode"].ToString(), body.ToString(), responseMsg.ToString(), "_委外生产订单变更单");
