...
{
    [Description("受托发料单审核--推送MES")]
    public class STFL_Send : AbstractOperationServicePlugIn
    {
        public override void EndOperationTransaction(EndOperationTransactionArgs e)
        {
            ...
            foreach (DynamicObject entity in e.DataEntitys)
            {
                ...
                if (entity != null)
                {
                    ...
                    string sql = string.Format(@"/*dialect*/ exec yzx_stfl '{0}'", FID);
                    DynamicObjectCollection data = DBUtils.ExecuteDynamicObject(this.Context, sql);
                   
                    string sqlzx = string.Format(@"/*dialect*/ exec yzx_stflEntry '{0}'", FID);
                    ...
                    list.Add(data);
                    list.Add(datazx);
                    string jar = JsonUtils.ObjectJsonst(list);

                    //待提供

                    var client = new RestClient(config[0]["http"].ToString() + "/api/v1/wms/openApi/commissioned");
                    client.Timeout = -1;
                    ...
                    Dbhple.Log.writestr("受托发料单：" + data[0]["applyCode"].ToString(), body.ToString(), responseMsg.ToString(), "_受托发料单");
