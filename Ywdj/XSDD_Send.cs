...
{
    [Description("销售订单审核--推送MES")]
    public class XSDD_Send : AbstractOperationServicePlugIn
    {
        public override void EndOperationTransaction(EndOperationTransactionArgs e)
        {
            ...
            foreach (DynamicObject entity in e.DataEntitys)
            {
                ...
                if (entity != null)
                {
                    ...
                    string sql = string.Format(@"/*dialect*/ exec yzx_xsdd '{0}'", FID);
                    DynamicObjectCollection data = DBUtils.ExecuteDynamicObject(this.Context, sql);
                    
                    string sqlzx = string.Format(@"/*dialect*/ exec yzx_xsddEntry '{0}'", FID);
                    ...
                    Dbhple.Log.writestr("销售订单：" + data[0]["orderCode"].ToString(), body.ToString(), responseMsg.ToString(), "_销售订单");
