...
{
    [Description("退货通知单审核--推送MES")]
    public class XSTH_Send : AbstractOperationServicePlugIn
    {
        public override void EndOperationTransaction(EndOperationTransactionArgs e)
        {
            ...
            foreach (DynamicObject entity in e.DataEntitys)
            {
                ...
                if (entity != null)
                {
                    ...
                    string sql = string.Format(@"/*dialect*/ exec yzx_xsth '{0}'", FID);
                    DynamicObjectCollection data = DBUtils.ExecuteDynamicObject(this.Context, sql);
                   
                    string sqlzx = string.Format(@"/*dialect*/ exec yzx_xsthEntry '{0}'", FID);
                    ...
                    Dbhple.Log.writestr("销售退货通知单：" + data[0]["orderCode"].ToString(), body.ToString(), responseMsg.ToString(), "_销售退货通知单");
