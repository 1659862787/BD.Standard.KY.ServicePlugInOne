...
{
    [Description("生产订单执行开工--推送MES")]
    public class SCYLQD_Send : AbstractOperationServicePlugIn
    {
        public override void EndOperationTransaction(EndOperationTransactionArgs e)
        {
            ...
            foreach (DynamicObject entity in e.DataEntitys)
            {
                ...
                if (entity != null)
                {
                    ...
                    string sql = string.Format(@"/*dialect*/ exec yzx_scylqd '{0}'", FID);
                    DynamicObjectCollection data = DBUtils.ExecuteDynamicObject(this.Context, sql);
                    
                    string sqlzx = string.Format(@"/*dialect*/ exec yzx_scylqdEntry '{0}'", FID);
                    ...
                    Dbhple.Log.writestr("生产用料清单：" + FBILLNO, body.ToString(), responseMsg.ToString(), "_生产用料清单");
