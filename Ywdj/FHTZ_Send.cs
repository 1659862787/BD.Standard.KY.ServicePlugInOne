...
{
    [Description("发货通知单审核--推送MES")]
    [Kingdee.BOS.Util.HotUpdate]
    public class FHTZ_Send: AbstractOperationServicePlugIn
    {
        public override void EndOperationTransaction(EndOperationTransactionArgs e)
        {
            ...
            foreach (DynamicObject entity in e.DataEntitys)
            {
                ...
                if (entity != null)
                {
                    ...
                    string sql = string.Format(@"/*dialect*/ exec yzx_fhtz '{0}'", FID);
                    DynamicObjectCollection data = DBUtils.ExecuteDynamicObject(this.Context, sql);
                    if (data.Count>0)
                    {

                        string sqlzx = string.Format(@"/*dialect*/ exec yzx_fhtzEntry '{0}'", FID);
                        ...
                        var client = new RestClient(config[0]["http"].ToString() + "/api/v1/wms/openApi/send/notice/syn");
                        client.Timeout = -1;
                        ...
                        Dbhple.Log.writestr("发货通知：" + data[0]["sendCode"].ToString(), body.ToString(), responseMsg.ToString(), "_发货通知");
