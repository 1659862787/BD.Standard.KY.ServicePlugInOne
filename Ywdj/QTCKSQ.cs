...
{
    [Description("出库申请单审核(其他出库)--推送MES")]
    public class QTCKSQ : AbstractOperationServicePlugIn
    {
        public override void EndOperationTransaction(EndOperationTransactionArgs e)
        {
            ...
            foreach (DynamicObject entity in e.DataEntitys)
            {
                ...
                if (entity != null)
                {
                    ...
                    string sql = string.Format(@"/*dialect*/ exec yzx_qtcksq '{0}'", FID);
                    DynamicObjectCollection data = DBUtils.ExecuteDynamicObject(this.Context, sql);
                   
                    string sqlzx = string.Format(@"/*dialect*/ exec yzx_qtcksqEntry '{0}'", FID);
                    ...
                    var client = new RestClient(config[0]["http"].ToString() + "/api/v1/wms/openApi/miscellaneous/apply/add");
                    client.Timeout = -1;
                    ...
                    Dbhple.Log.writestr("其他出库申请单：" + FBILLNO, body.ToString(), responseMsg.ToString(), "_其他出库申请单");
