...
{
    [Description("物料清单BOM审核--推送MES")]
    public class BOM_Send : AbstractOperationServicePlugIn
    {
        public override void EndOperationTransaction(EndOperationTransactionArgs e)
        {
            ...
            foreach (DynamicObject entity in e.DataEntitys)
            {
                ...
                if (entity != null)
                {
                    ...
                    string sql = string.Format(@"/*dialect*/ exec yzx_bom '{0}'", FID);
                    DynamicObjectCollection data = DBUtils.ExecuteDynamicObject(this.Context, sql);
                    
                    string sqlzx = string.Format(@"/*dialect*/ exec yzx_bomzx '{0}'", FID);
                    ...
                    var client = new RestClient(config[0]["http"].ToString() + "/api/v1/mds/openApi/bom/syn");
                    client.Timeout = -1;
                    ...
                    Dbhple.Log.writestr("BOM：" + data[0]["bomCode"].ToString(), body.ToString(), responseMsg.ToString(), "_BOM");
